...
import React, { lazy, Suspense, type JSX, memo, type ReactNode } from "react";
...
import { shouldOfferSignInWithQrOnLogin } from "../../../utils/shouldOfferSignInWithQrOnLogin";
import { Mode } from "../../views/auth/LoginWithQR-types";
import { ModuleApi } from "../../../modules/Api.ts";

const LoginWithQR = lazy(() => import("../../views/auth/LoginWithQR"));
...
interface IState {
    ...
    /** MSC4108 + OIDC device grant available for QR sign-in (locked-server deployments). */
    qrLoginAvailable: boolean;
    signInWithQrMode: boolean;
}
...
class LoginComponent extends React.PureComponent<IProps, IState> {
            ...
            .then(
                (flows) => {
                    ...
                    if (supportedFlows.length === 0) {
                        ...
                        this.updateQrLoginAvailability();
                    }
                },
                (err) => {
                    this.setState({
    ...
    private updateQrLoginAvailability = (): void => {
        if (!this.loginLogic || !SdkConfig.get("disable_custom_urls")) {
            if (!this.unmounted) {
                this.setState({ qrLoginAvailable: false });
            }
            return;
        }
        ...
        this.loginLogic
            ...
            .then((versions) => {
                if (this.unmounted) return;
                const available = shouldOfferSignInWithQrOnLogin(
                    versions,
                    this.props.serverConfig.delegatedAuthentication,
                );
                this.setState({ qrLoginAvailable: available });
            })
            .catch(() => {
                if (!this.unmounted) {
                    this.setState({ qrLoginAvailable: false });
    ...
    private onShowSignInWithQr = (): void => {
        this.setState({ signInWithQrMode: true });
    };

    private onQrFinished = (success: boolean): void => {
        this.setState({ signInWithQrMode: false });
        if (success) {
    ...
    public render(): React.ReactNode {
        if (this.state.signInWithQrMode && this.loginLogic) {
            return (
                <Suspense
                    fallback={
                ...
                >
                    <LoginWithQR
                        mode={Mode.Show}
                        onFinished={this.onQrFinished}
                        client={this.loginLogic.createTemporaryClient()}
                    />
                </Suspense>
            );
        }
        ...
        return (
            <AuthPage>
                ...
                <AuthBody>
                    ...
                    !this.isBusy() &&
                    this.state.qrLoginAvailable &&
                    this.state.flows &&
                    this.state.flows.length > 0 ? (
                        <AccessibleButton
                            className="mx_Login_fullWidthButton"
                            kind="secondary"
                            onClick={this.onShowSignInWithQr}
                        >
