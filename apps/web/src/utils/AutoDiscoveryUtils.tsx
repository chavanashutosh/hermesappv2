...
export default class AutoDiscoveryUtils {
    ...
    public static authComponentStateForError(err: unknown, pageName = "login"): IAuthComponentState {
        ...
        const lockedServerCopy = SdkConfig.get("disable_custom_urls");

        let title = _t("cannot_reach_homeserver");
        let body: ReactNode = _t("cannot_reach_homeserver_detail");
        if (lockedServerCopy && AutoDiscoveryUtils.isLivelinessError(err)) {
            title = _t("auth|locked_server_unavailable_title");
            body = _t("auth|locked_server_unavailable_detail");
        } else if (lockedServerCopy && !AutoDiscoveryUtils.isLivelinessError(err)) {
            title = _t("auth|locked_server_misconfigured_title");
            body = _t("auth|locked_server_misconfigured_detail");
        } else if (!AutoDiscoveryUtils.isLivelinessError(err)) {
